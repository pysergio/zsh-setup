# Generated by Powerlevel10k configuration wizard on 2024-07-11 at 17:26 EEST.
...
() {
  ...
  function my_git_formatter() {
    ...
    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      ...
      # If local branch name is at most 42 characters long, show it in full.
      # Otherwise show the first 17 … the last 17.
      # Tip: To always show local branch name in full without truncation, delete the next line.
      (( $#branch > 42 )) && branch[18,-18]="…"  # <-- this line
    ...
    if [[ -n $VCS_STATUS_TAG
      ...
      VCS_TEST="$tag"
      # If tag name is at most 42 characters long, show it in full.
      # Otherwise show the first 17 … the last 17.
      # Tip: To always show tag name in full without truncation, delete the next line.
      (( $#tag > 42 )) && tag[18,-18]="…"  # <-- this line
      res+="${meta}#${clean}${tag//\%/%%}"
    fi
    ...
    if (( VCS_STATUS_COMMITS_AHEAD && VCS_STATUS_COMMITS_BEHIND )); then
      res+=" ${conflicted}<>${VCS_STATUS_COMMITS_BEHIND}"
    elif (( VCS_STATUS_COMMITS_BEHIND )); then
      res+=" ${modified}<${VCS_STATUS_COMMITS_BEHIND}"
    elif (( VCS_STATUS_COMMITS_AHEAD )); then
      res+=" ${untracked}>${VCS_STATUS_COMMITS_AHEAD}"
    else
    ...
    #   (( VCS_STATUS_COMMITS_BEHIND )) && res+=" ${_gscolor}⇣${VCS_STATUS_COMMITS_BEHIND}"
    #   # ⇡42 if ahead of the remote; no leading space if also behind the remote: ⇣42⇡42.
    #   (( VCS_STATUS_COMMITS_AHEAD && !VCS_STATUS_COMMITS_BEHIND )) && res+=" "
    #   (( VCS_STATUS_COMMITS_AHEAD  )) && res+="${_gscolor}⇡${VCS_STATUS_COMMITS_AHEAD}"
